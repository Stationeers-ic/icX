...
export class ConstantAssignment extends Token implements TokenInterface {
	readonly type = TOKEN_TYPES.ConstantAssignment
	...
	static is(token: TokenInterface | null): token is ConstantAssignment {
		if (token === null) return false
		return token.type === TOKEN_TYPES.ConstantAssignment
	}
	static parse(tokens: LexerToken[], parent: TokenInterface): [TokenInterface, LexerToken[]] | null {
		...
		const newToken = new ConstantAssignment(included[0], included.slice(2), parent)
		return [newToken, other]
	}
}

export default ConstantAssignment
