...
export class ConstantDeclarator extends Token implements TokenInterface {
	readonly type = TOKEN_TYPES.ConstantDeclarator
	...
	static is(token: TokenInterface | null): token is ConstantDeclarator {
		if (token === null) return false
		return token.type === TOKEN_TYPES.ConstantDeclarator
	}
	static parse(tokens: LexerToken[], parent: TokenInterface): [TokenInterface, LexerToken[]] | null {
		...
		const newToken = new ConstantDeclarator(after, parent)
		return [newToken, other]
	}
}

export default ConstantDeclarator
