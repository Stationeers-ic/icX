...
export class ContinueNode extends ASTNode {
	readonly type = AST_NODE_TYPE.CONTINUE
	constructor(
	...
	static create(tokens: CODE_TOKEN[], errors: ErrorMessage[]): null | [node: ContinueNode, next: CODE_TOKEN[]] {
		if (tokens[0].type !== TOKEN_TYPES.CONTINUE) return null
		...
		for (let i = 1; i < tokens.length; i++) {
			if (tokens[i].type === TOKEN_TYPES.NEWLINE) {
				...
				return [new ContinueNode(token.start, token.end, comment), tokens.slice(i + 1)]
			}
		...
		return [new ContinueNode(token.start, token.end), []]
	}
}
