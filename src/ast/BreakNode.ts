...
export class BreakNode extends ASTNode {
	readonly type = AST_NODE_TYPE.BREAK
	constructor(
	...
	static create(tokens: CODE_TOKEN[], errors: ErrorMessage[]): null | [node: BreakNode, next: CODE_TOKEN[]] {
		if (tokens[0].type !== TOKEN_TYPES.BREAK) return null
		...
		for (let i = 1; i < tokens.length; i++) {
			if (tokens[i].type === TOKEN_TYPES.NEWLINE) {
				...
				return [new BreakNode(token.start, token.end, comment), tokens.slice(i + 1)]
			}
		...
		return [new BreakNode(token.start, token.end), []]
	}
}
